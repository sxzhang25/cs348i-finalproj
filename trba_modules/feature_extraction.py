"""Feature extractor module for TRBA text recognition model.
...
class VGG_FeatureExtractor(nn.Module):
  ...
  def __init__(self, input_channel, output_channel=512):
      ...
      self.ConvNet = nn.Sequential(
        ...
        nn.MaxPool2d(2, 2),  # 128 x 8 x 25
        nn.Conv2d(self.output_channel[1], self.output_channel[2], 3, 1, 1), nn.ReLU(True),  # 256 x 8 x 25
        nn.Conv2d(self.output_channel[2], self.output_channel[2], 3, 1, 1), nn.ReLU(True),
        nn.MaxPool2d((2, 1), (2, 1)),  # 256 x 4 x 25
        nn.Conv2d(self.output_channel[2], self.output_channel[3], 3, 1, 1, bias=False),
        nn.BatchNorm2d(self.output_channel[3]), nn.ReLU(True),  # 512 x 4 x 25
        nn.Conv2d(self.output_channel[3], self.output_channel[3], 3, 1, 1, bias=False),
        nn.BatchNorm2d(self.output_channel[3]), nn.ReLU(True),
        nn.MaxPool2d((2, 1), (2, 1)),  # 512 x 2 x 25
        nn.Conv2d(self.output_channel[3], self.output_channel[3], 2, 1, 0), nn.ReLU(True))  # 512 x 1 x 24
...
class GRCL(nn.Module):
  ...
  def forward(self, input):
    """Forward pass through GRCL.

    The input of GRCL is consistant over time t, which is denoted by u(0), thus 
    wgf_u / wf_u is also consistant over time t.
    """
